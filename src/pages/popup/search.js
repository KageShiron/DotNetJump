...
export default {
    ...
    ,
        searchNetCoreSource: async function (name) {
        if (!name && name === "") return null;
        console.log("searchNetCoreSource:" + name);
        const url = `https://source.dot.net/api/symbols/?symbol=${name}`;
        const res = (await (await fetch(url)).text());
        const alist = new DOMParser().parseFromString(`<base href="https://source.dot.net/" />` + res, "text/html").querySelectorAll(".resultGroup>div:not(.resultGroupHeader)>a");
        const list = [];
        console.log(alist);
        for (let e of alist) {
            const desc = e.querySelector(".resultDescription");
